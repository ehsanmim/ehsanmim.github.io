    ...
    >
      <ol className="space-y-px">
        {experience.map((job, i) => (
          <Reveal as="li" key={`${job.company}-${job.start}`} delay={i * 80}>
            <article className="group grid gap-4 border-b border-rule py-8 transition-colors duration-300 hover:border-accent-dim sm:grid-cols-[11rem_1fr] sm:gap-8">
              <div className="pt-1">
                <div className="font-mono text-xs tracking-wide text-ink-faint transition-colors duration-300 group-hover:text-accent">
                  {period(job, lang, present)}
                </div>
                <div className="mt-1.5 text-xs text-ink-faint">{t(job.location)}</div>
              </div>

              <div>
                <h3 className="display text-2xl text-ink sm:text-3xl">{t(job.role)}</h3>
      ...
      <Reveal className="mt-14">
        ...
        <ul className="mt-5 space-y-4">
          {education.map((item) => (
            <li key={t(item.what)} className="grid gap-1 sm:grid-cols-[11rem_1fr]">
              <span className="font-mono text-xs text-ink-faint">
                {period(item, lang, present)}
              </span>
              <span>
                <span className="block text-[0.9375rem] text-ink">{t(item.what)}</span>
                {item.where && (
